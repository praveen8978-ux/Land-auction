...
export default function NewListingPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (coords) {
        data.append('lat', coords.lat.toString());
        data.append('lng', coords.lng.toString());
      }
      ...
      router.push('/seller/pay?listed=true');
    } catch (err: unknown) {
      if (err && typeof err === 'object' && 'response' in err) {
        const axiosErr = err as { response?: { data?: { error?: string } } };
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  Pin location for proximity alerts
                </label>
                {coords ? (
                  ...
                  <button
                    ...
                    className="w-full border border-gray-200 text-gray-600 py-2.5 rounded-lg text-sm hover:bg-gray-50 disabled:opacity-60 flex items-center justify-center gap-2"
                  >
                    {detectingLocation ? 'Detecting...' : '📍 Pin land location (for buyer alerts)'}
                  </button>
                )}
                <p className="text-xs text-gray-400 mt-1">
                  Buyers within 50km will be notified when you list this land
                </p>
              </div>
            </div>
          </div>
              ...
              <div className="border-t border-gray-100 pt-4">
                <div className="flex items-center justify-between mb-2">
                  <div>
                    ...
                    <p className="text-xs text-gray-400">Get an AI estimate based on your land details</p>
                  </div>
                  <button
                    type="button" onClick={handleValuate} disabled={valuating}
                  ...
                  >
                    {valuating ? (
                      <><span className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin inline-block"/>Analysing...</>
                    ) : 'Get AI estimate'}
                  </button>
                </div>
                {valuationError && <p className="text-red-600 text-xs mt-2">{valuationError}</p>}
                {valuation && <ValuationCard valuation={valuation} areaUnit={formData.areaUnit} />}
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-6">
            ...
            <div className="grid grid-cols-3 gap-4">
              {[
                { name: 'roadAccess',  label: 'Road access',  checked: formData.roadAccess  },
                { name: 'waterSource', label: 'Water source', checked: formData.waterSource },
                { name: 'electricity', label: 'Electricity',  checked: formData.electricity },
              ].map(a => (
                <label key={a.name} className="flex items-center gap-3 cursor-pointer">
                  <input
                    type="checkbox" name={a.name} checked={a.checked}
                    onChange={handleChange}
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-6">
            ...
            <p className="text-xs text-gray-400 mb-5">Upload up to 6 photos. JPG, PNG or WEBP. Max 5MB each.</p>
            {previews.length > 0 && (
            ...
            {photos.length < 6 && (
              <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-gray-200 rounded-xl cursor-pointer hover:border-blue-400 hover:bg-blue-50 transition-colors">
                ...
                <input type="file" multiple accept="image/jpeg,image/png,image/webp" onChange={handlePhotos} className="hidden"/>
              </label>
            )}
          </div>
