...
export default function AuctionDetailPage() {
  ...
  useEffect(() => {
    ...
    socket.on('newBid', (data: any) => {
      setAuction(prev => prev ? { ...prev, currentPrice: data.amount, totalBids: data.totalBids } : prev);
      setBids(prev => [{
  ...
  const handleBid = async () => {
    ...
    try {
      ...
      setBidSuccess(`Bid of ₹${Number(bidAmount).toLocaleString('en-IN')} placed successfully!`);
      setBidAmount('');
    } catch (err: any) {
          ...
          <div className="lg:col-span-2 space-y-5">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-6">
              <div className="flex items-start justify-between mb-4">
                <h1 className="text-xl font-bold text-gray-900">{auction.land.title}</h1>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-6">
              ...
              <div className={`text-center py-3 rounded-xl mb-4 ${
                ...
                <p className={`text-2xl font-bold tabular-nums ${
                  isLive ? 'text-green-700' : isEnded ? 'text-gray-500' : 'text-blue-700'
                }`}>
                  {isLive ? timeLeft : isEnded ? 'Ended' : `Starts ${new Date(auction.startTime).toLocaleDateString('en-IN')}`}
                </p>
              ...
              {canBid && (
                <div>
                  ...
                  <p className="text-xs text-gray-400 mb-2">Quick bid</p>
                  <div className="grid grid-cols-3 gap-2 mb-3">
                    {[10000, 25000, 50000].map(inc => (
                      <button
                        key={inc}
                        onClick={() => suggestBid(inc)}
                        className="bg-blue-50 text-blue-700 text-xs font-semibold py-2 rounded-lg hover:bg-blue-100"
                      >
                  ...
                  {bidError && (
                    <p className="text-red-600 text-xs mb-2">{bidError}</p>
                  )}
                  {bidSuccess && (
                    <p className="text-green-600 text-xs mb-2">{bidSuccess}</p>
                  )}
              ...
              {!user && (
                <Link href="/login" className="block w-full text-center bg-blue-600 text-white py-3 rounded-xl font-semibold hover:bg-blue-700">
                  Login to bid
                </Link>
              )}
              ...
              <div className="flex items-center gap-2 mt-3">
                ...
                <p className="text-xs text-gray-400">{connected ? 'Live updates connected' : 'Connecting...'}</p>
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-5">
              ...
              {bids.length === 0 ? (
                ...
                <div className="space-y-2 max-h-64 overflow-y-auto">
                  {bids.map((bid, i) => (
                    <div key={bid._id} className={`flex items-center justify-between py-2 px-3 rounded-xl ${i === 0 ? 'bg-blue-50' : 'bg-gray-50'}`}>
                      <div>
